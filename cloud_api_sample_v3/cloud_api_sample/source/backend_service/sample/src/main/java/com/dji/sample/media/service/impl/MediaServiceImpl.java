...
public class MediaServiceImpl extends AbstractMediaService implements IMediaService {
    ...
    @Override
    public MediaUploadCallbackRequest convert2callbackRequest(MediaUploadCallbackRequest request) {
        return request;
    }

    @Override
    public void saveMediaMissionRef(MediaUploadCallbackRequest file) {
        MediaMissionRefEntity mediaMissionRef = new MediaMissionRefEntity();
        mediaMissionRef.setFileName(file.getName());
        mediaMissionRef.setMissionId(missionService.getMissionIdByDeviceSn(file.getExt().getSn()));
        mediaMissionRef.setLongitude(file.getMetadata().getShootPosition().getLng());
        mediaMissionRef.setLatitude(file.getMetadata().getShootPosition().getLat());
        mediaMissionRef.setAltitude(file.getMetadata().getAbsoluteAltitude());
        mediaMissionRef.setRelativeAltitude(file.getMetadata().getRelativeAltitude());
        mediaMissionRef.setGimbalYawDegree(file.getMetadata().getGimbalYawDegree());
        mediaMissionRefService.createMediaMissionRef(mediaMissionRef);
        log.debug("Media file {} linked to mission", file.getName());
    }
}
